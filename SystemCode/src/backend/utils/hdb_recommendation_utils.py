...
def score_flat_type(user_pref_flat_num: float, item_flat_type: str, sigma: float = 0.8) -> float:
    """Preferred_Flat_Area_or_Room_Num (1..7) vs flat_type (mapped) → [0,1]."""
...
def compute_match_scores_gaussian(user_row: dict | pd.Series,
    ...
    return {
        "sim_flat_type": score_flat_type(user_row["Preferred_Flat_Area_or_Room_Num"], item_row["flat_type"], s["flat"]),
...
def _weighted_label_from_feats(feats_full: dict, user: dict | pd.Series) -> float:
    ...
    w_flat_type  = 0.60 + 0.60 * _p(user.get("Preferred_Flat_Area_or_Room_Num", 3))  # proxy; no explicit 'flat' priority
...
def recommend_for_user(model,
    ...
    #       f"flat_type={user_summary['Preferred_Flat_Area_or_Room_Num']}, floor_pref={user_summary['Floor_Preference']}, "
    ...
    lines.append(
        ...
        f"flat_type={user_summary.get('Preferred_Flat_Area_or_Room_Num')}, "
