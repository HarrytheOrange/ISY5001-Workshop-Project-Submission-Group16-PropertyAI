...
def recommend_for_user(model,
    ...
    lines.append(
        ...
        f"area={user_summary.get('Preferred_Flat_Area')}, "
    ...
    for item in items_struct:
        ...
        if a.get("floor_area_sqm") or a.get("remaining_lease"):
            lines.append(
                f"   Area: {a.get('floor_area_sqm', 'NA')} sqm   "
                f"Lease: {a.get('remaining_lease', 'NA')}"
            )
