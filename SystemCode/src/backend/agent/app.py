...
def warp_predict_condo_price(house_info: dict) -> float:
    """
        ...
        """
    print("Calling condo price prediction with info:\n", house_info)
...
def warp_predict_hdb_price(house_info: dict) -> float:
    ...
    """
    print("Calling HDB price prediction with info:\n", house_info)
...
def warp_get_sg_route(inputs: dict) -> dict:
    ...
    a = inputs.get("start")
    b = inputs.get("end")
    print(f"Generating route from {a} to {b}")
...
def warp_hdb_recommend(inputs: dict) -> dict:
    ...
    """
    print("Generating HDB recommendations with inputs:\n", inputs)
...
def warp_condo_recommend(inputs: dict) -> dict:
    ...
    {
        "Budget_SGD": 4000000,
        "Preferred_Flat_Area_or_Room_Num": 700,
        "Preferred_Place": "JURONG EAST",
        "Priority_Distance_Proximity": 0.8,
        "Floor_Preference": 2,
        "NewHome_Preference": 0.5,
    ...
    Returns a JSON of top-N recommendations.
    """
    print("Generating Condo recommendations with inputs:\n", inputs)
...
class UserNeedGraph:
    ...
    def chatbot(self, state: State):
        ...
        {state["user_needs"]}
        You can ask for missing ones. Each time, ask for only one missing field.
        If the user do not have a explicit need, you can help to suggest based on the known needs.
        Ask in the tone of a friendly and professional real estate agent.
        If you learn a new value, call the update_user_need tool.

        If the user has specific questions about property prices, routes, or recommendations, do not ask for more needs.
        Use the appropriate tools to assist them.
        If you do not have enough information to use the tools, just make up reasonable example inputs for the tools.

        Your proirity is to make the system usable.
        If you do not have certain ability or information to answer some questions, always use you knowledge to give a plausible answer, do not say you do not know.
        Even if the tools fail, still give a plausible answer based on your knowledge.
        ...
        {{
            ...
            "Floor_Preference": 2,
            "NewHome_Preference": 0.5,
            "Priority_MRT_Access": 3,
            "Priority_Bus_Access": 3,
            "Priority_Amenities": 3,
            "Priority_School_Proximity": 3,
            "Priority_Park_Access": 3
        }} (If user not provide, you can make up a reasonable example.)
        ...
        #### 1. Selling HDB with Price Check
        **User:** I’m planning to sell my 4-room flat at 640627, 11th floor. What’s the current market price and is now a good time to sell?  
        **System:**  
        Based on recent HDB resale transactions at *Blk 627 Jurong West Street 65*, 4-room units on high floors transacted around **$620k–$650k** in **Q3 2025**.  
        Market momentum is **moderate (-0.3% QoQ)**.  
        If you’re aiming for max profit, data suggests listing between **Nov–Feb 2026** for stronger demand before new BTO launches.

        ---

        #### 2. Price Prediction Transparency
        **User:** Estimate my 3-room HDB at 561201, 8th floor.  
        **System:**  
        Predicted value = **$448k ± $12k**,  
        based on model factors:  
        • floor (+4%) • lease remaining (-6%) • distance to MRT (+3%)  
        Comparable units sold for **$440k–$460k** over the past 3 months.

        ---

        #### 3. Buying Near MRT and Food Courts
        **User:** Show me 3-bedroom condos within 1 km of Thomson MRT, near schools.  
        **System:**  
        **Options:**  
        - **Thomson Three** – 650 m to MRT, near Ai Tong School  
        - **Sky Vue** – direct Bishan MRT access, within CHIJ Pri zone  
        - **JadeScape** – 700 m to Upper Thomson MRT, near Raffles Institution  
        All priced in the **$1.3M–$1.6M** range.

        ---

        #### 4. Affordability Planner (First-Time Buyer)
        **User:** I earn $6,000 a month and my partner earns $5,000. We want a 4-room resale flat. How much can we afford?  
        **System:**  
        Combined income = **$11,000** → MSR 30% → **max loan ≈ $650k**.  
        With typical CPF + cash ≈ **$150k** downpayment, safe budget ≈ **$780k**.  
        You can consider **Bukit Panjang** or **Sengkang** 4-room units.

        ---

        #### 5. Upgrading HDB → Condo
        **User:** I own a 4-room HDB at Punggol Central worth about $580k. I want to upgrade to a condo around $1.2M. Can I afford it?  
        **System:**  
        Assuming household income = **$9,000/month**, no other loans:  
        Estimated max loan ≈ **$960k (TDSR 60%)**  
        Cash + CPF from sale ≈ **$580k**  
        You can afford properties up to **~$1.5M**.  
        Suggested: **Treasure at Tampines** or **Piermont Grand EC**.

        ---

        #### 6. Comparative Recommendation
        **User:** Compare 2 properties — Hillock Green vs Lentor Modern.  
        **System:**  
        **Hillock Green:** newer, lower PSF ($2,200), slightly further from MRT.  
        **Lentor Modern:** integrated development with mall & MRT direct access, higher rentability.  
        For long-term capital gain, **Lentor Modern** projects **+8% ROI vs +5%** for Hillock.

        ---

        #### 7. School Zone Search
        **User:** Find properties within 1 km of Nan Hua Primary and under $1.4M.  
        **System:**  
        Recommended:  
        - **Clementi Peaks** 4-room HDB (~$920k)  
        - **Parc Clematis** 2-bed (~$1.35M)  
        Both within **Nan Hua Primary priority zone** and under budget.
